...
run_time = 300  # 运行次数
sleep_time = 0.5  # 延迟时间
xlsfilepath = 'patent.xls'  # 读入文件
path = 'output2.xls'  # 输出文件


def get_companylist(path):
    with xlrd.open_workbook(path) as f:  # 打开excel文件
        sheet = f.sheet_by_index(0)
        infolist = sheet.col_values(2)[1:]  # 读取第三列（公司名称）
    return infolist
...
def get_data(runtime, sleeptime):
    ...
    for i in range(runtime):
        ...
        patenttypes = driver.find_elements(By.CLASS_NAME, 'pills-item')[8:12]  # 获取所有专利信息 1
